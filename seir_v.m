...
%                 beta2_list(j) = 0;
%                 den_i = den_i + 1;
%             elseif beta2d_list(j) == 0
%                 beta2d_list(j) = 0;
%                 dedn = dedn + 1;
%             else
%                 disp("error beta_list(j)")
%             end
        
        % beta2 not zero element, intensity update
        temp = (k1*log(k2*E_con(mask~=0) + 1));
        beta2_list(mask~=0) = temp;
%         beta2d_list(beta2d_list > 0) = temp(beta2d_list > 0);
    end
    p1_t = min(p1 + dep, N);
    mask(p1:p1_t) = 1;
    p1 = p1_t;
    ...
    if E(i) > 0
        ...
        mask(idx_cho)=1;
%         beta2d_list(idx_cho) = beta2_list(idx_cho);
%         beta2_list(idx_cho) = 0;
    else
        num = 0; 
    end
